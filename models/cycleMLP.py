...
class CycleBlock(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=2*dim, out_features=4*dim, act_layer=act_layer)
        self.skip_lam = skip_lam
        self.mlp_x = Mlp(in_features=dim, hidden_features=2*dim, out_features=4*dim, act_layer=act_layer)

    def forward(self, x):
        x = x + self.drop_path(self.attn(self.norm1(x))) / self.skip_lam
        x = self.mlp_x(x) + self.drop_path(self.mlp(self.norm2(x))) / self.skip_lam
        return x
