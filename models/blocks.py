...
class MLPUpsampler(nn.Sequential):
    def __init__(self, scale, in_dim, out_dim, hidden_list):
        super(MLPUpsampler, self).__init__()
        self.scale = scale
        if scale == 2:    
            lastv = in_dim
            m = []
            for hidden in hidden_list:
                m.append(nn.Linear(lastv, hidden))
                m.append(nn.GELU()) 
                lastv = hidden
            m.append(nn.Linear(lastv, out_dim))
            ...
            for hidden in hidden_list:
                ...
                lastv = hidden
            m.append(nn.Linear(lastv, out_dim))
            self.layers1 = nn.Sequential(*m)
            self.rearrange1 = nn.PixelShuffle(2)
            m = []
            lastv = in_dim
            for hidden in hidden_list:
                m.append(nn.Linear(lastv, hidden))
                ...
                lastv = hidden
            m.append(nn.Linear(lastv, out_dim))
            self.layers2 = nn.Sequential(*m)
            self.rearrange2 = nn.PixelShuffle(2)
        elif scale == 8:    
            lastv = in_dim
            m = []
            for hidden in hidden_list:
            ...
            for hidden in hidden_list:
                ...
                lastv = hidden
            m.append(nn.Linear(lastv, out_dim))
            self.layers3 = nn.Sequential(*m)
            self.rearrange3 = nn.PixelShuffle(2)

    def forward(self, x):   
        if self.scale == 2:     
            [_B, _C, _H, _W] = x.size()
            x = x.permute(0,2,3,1)
            x = self.layers1(x.reshape(-1, x.shape[-1]))
            shape = [_B, _H, _W, _C*4]
            x = x.reshape(shape)
            x = x.permute(0,3,1,2)
            x = self.rearrange1(x)
            ...
            [_B, _C, _H, _W] = x.size()
            x = x.permute(0,2,3,1)
            x = self.layers2(x.reshape(-1, x.shape[-1]))
            shape = [_B, _H, _W, _C*4]
            x = x.reshape(shape)
            x = x.permute(0,3,1,2)
            x = self.rearrange2(x)
            ...
            [_B, _C, _H, _W] = x.size()
            x = x.permute(0,2,3,1)
            x = self.layers3(x.reshape(-1, x.shape[-1]))
            shape = [_B, _H, _W, _C*4]
            x = x.reshape(shape)
            x = x.permute(0,3,1,2)
            x = self.rearrange3(x)
